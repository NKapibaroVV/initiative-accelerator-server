    ...
    if (err) {
      ...
      pool.query(`CREATE TABLE \`initiatives_${login}\` (
        ...
        if (err) {
          ...
          pool.query(`SELECT \`name\`,\`surname\`, \`id\`, \`token\`, \`birth\`, \`role\`, \`score\` FROM \`users\` WHERE \`email\`=${mysql.escape(email)} AND \`name\`=${mysql.escape(first_name)} AND \`surname\`=${mysql.escape(second_name)} AND \`login\`=${mysql.escape(login)}`, function (err: any, result: any) {
            if (err) {
              res.send(err)
            } else {
              res.send(result)
