            ...
            if (shopItem.user_id != null) {
              ...
              pool.query(query, function (err: any, result: any) {
                if (err) {
                  ...
                  res.send({0:result, 1:shopItem, 2:resultShopItem, 3:query})
                }
              })
            }
            else {
...
expressApp.post("/api/update_profile/", (req: any, res: any) => {
  ...
  pool.query(`SELECT \`name\`,\`surname\`, \`login\`, \`id\`, \`token\`, \`birth\`, \`role\`, \`score\`, \`email\`, \`password\` FROM \`users\` WHERE \`token\`=${mysql.escape(token)}`, function (err: any, result: any) {
    if (err) {
      ...
      let sql = `UPDATE \`users\` SET \`name\`=${mysql.escape(name)}, \`surname\`=${mysql.escape(surname)}, \`email\`=${mysql.escape(email)}, \`edu_group\`=${mysql.escape(edu_group)}, \`birth\`=${mysql.escape(birth)}${!!password ? `, \`password\`=${mysql.escape(SHA512(password).toString())}` : ""}${!!avatarURI ? `, \`avatarURI\`=${mysql.escape(avatar)}` : ""} WHERE \`id\`='${user.id}'`
